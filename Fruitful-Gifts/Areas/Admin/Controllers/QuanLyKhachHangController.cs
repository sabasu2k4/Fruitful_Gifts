...
public class QuanLyKhachHangController : Controller
{
    ...
    public async Task<IActionResult> QLKhachHang(int? page, string searchString, string sortOrder, string trangThai)
    {
        int pageSize = 8;
        ...
        {
            int status = int.Parse(trangThai);
            customers = customers.Where(c => c.TaiKhoan.TrangThai == status);
        }
        ...
        switch (sortOrder)
        {
            ...
            case "date_desc":
                ...
                break;
            case "Status":
                customers = customers.OrderBy(c => c.TaiKhoan.TrangThai);
                break;
            case "status_desc":
                customers = customers.OrderByDescending(c => c.TaiKhoan.TrangThai);
                break;
            default:
        ...
        ViewBag.TotalLockedAccounts = await _context.TaiKhoans
        .CountAsync(t => t.TrangThai == 0 && t.VaiTro == "KhachHang");

        ViewBag.TotalActiveAccounts = await _context.TaiKhoans
        .CountAsync(t => t.TrangThai == 1 && t.VaiTro == "KhachHang");
    ...
    public async Task<IActionResult> DetailsKH(int id, int pageNumber = 1)
    {
        ...
        // Điều chỉnh trang nằm trong giới hạn
        if (pageNumber < 1) pageNumber = 1;
        if (pageNumber > totalPages) pageNumber = totalPages;
