...
namespace Fruitful_Gifts.Libraries
{
    public class VnpayLibrary
    {
        ...
        public PaymentResponseModel GetFullResponseData(IQueryCollection collection, string hashSecret)
        {
            ...
            if (!checkSignature)
            {
                // Gán ResponseCode để Success trả về false
                return new PaymentResponseModel()
                {
                    ResponseCode = "Invalid" // hoặc bất cứ giá trị nào != "00"
                };
            }

            // Parse amount
            double amount = 0;
            if (long.TryParse(amountRaw, out var amountLong))
            {
                amount = amountLong / 100.0; // VNPay trả về *100
            }
        ...
        private string HmacSha512(string key, string inputData) //mã hóa 
        {
